...
public class ChannelInviteServiceImpl implements ChannelInviteService {
    ...
    public ChannelInviteServiceImpl(ChannelInviteRepository repository, AuthContext authContext, MemberService memberService, UserService userService, ChannelService channelService) {
        ...
        if (memberService.findByUser(authUserId).getRole().
            ...
            if (!repository.existsByChannel_NameAndRecipient_Id(channelName, recipientId)) {
                ChannelInvite channelInvite = new ChannelInvite(
                        memberService.findByUser(authUserId),
                        channelService.getBroadcastChannelByName(channelName),
